"""This script is designed to identify the all the IP addresses on each host in the host file
create a target list using that data and get each host to then ping each IP address. It 
is set to ignore interface GigabitEthernet0/0 as MGMT interface"""
import getpass
import time
...
rprint("[bold red on yellow]***THIS SCRIPT WILL PING ALL IP ADDRESSES ON All INTERFACES FROM ALL HOSTS, EXCEPT MGMT INTERFACE***[/bold red on yellow]")
time.sleep(2)
